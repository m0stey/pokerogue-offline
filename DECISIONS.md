...
a usable version for the user that's stable and secure"), the following was decided and is now implemented
...
- **The app no longer updates itself.** The downloading updater is gone — no release download, no
  ...
  German message asking the user to get the new installation. A new game version arrives as a new
...
- **SECURITY.md** is written and states the two accepted weaknesses plainly: the offline login does
  not verify the password, and anyone with the Windows account has everything. The installer is
  unsigned.
