...
game-build/               Script to build the game from an upstream tag (local and .github/workflows/release.yml)
...
- Login: the game's own login screen (first run only; offline replay keeps her logged in).
...
- Dialogs: the conflict question; the "your progress is safe" notice on an unrecoverable rejection; the update window (below); and one "new version is being prepared" notice, shown at most once per app start when the proxy sees a save newer than the served build or `SyncResult.needsGameUpdate` is set, and no newer release exists yet.
...
- **Updater: automatic, whole installer.** On every start (as soon as the connection is up) and every 6 h it reads the GitHub releases of `m0stey/pokerogue-offline` (`release-<gameTag>-app<appVersion>`, assets `PokeRogue-Setup.exe`, `.sha256`, `release.json`). A newer game tag, or the same game with a newer app version, starts the download at once with a small progress window; she can keep playing and hide the window. The download must come from GitHub over https and match the published SHA-256 and size, otherwise it is deleted. Once verified, "Jetzt neu starten" or simply closing the game runs the last sync and exits; a hidden PowerShell waits for the process to end and runs the installer with `/S --updated --force-run`, which replaces the program folder and starts PokeRogue again. Saves, backups and settings live outside the program folder and are untouched. Never downgrades. The same check runs at once when a save needs a newer game. (Re-added 2026-09-13 at the owner's request; replaces the notice-only updater.)
