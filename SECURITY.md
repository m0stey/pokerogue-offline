...
One thing, really: **her PokéRogue account**. Concretely:

| Thing | Where it lives |
|---|---|
| The account token (what the game sends instead of the password) | `%APPDATA%\PokeRogue Offline\mirror\account.json` |
| Her saves — the profile and up to five runs | `%APPDATA%\PokeRogue Offline\mirror\*.json` |
...
*username* into the game's login screen is enough to get back in. Anyone who can already use her
Windows account can therefore open the game as her.

**2. Anyone with access to her Windows account has access to everything.** The token encryption is
...
Both are accepted deliberately. Fixing either would mean asking her for a password the app cannot
verify, which trades a real problem she does not have for a daily annoyance she would.
...
downloaded from somewhere she found herself. An unsigned installer cannot prove it came from us, so
