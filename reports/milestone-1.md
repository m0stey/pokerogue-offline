...
project got a bit too big — I want a usable version for her that's stable and secure". Steps 4 and
...
**Is it a risk for her account? Yes, and it is the worst failure mode we have.** The moment she
opens pokerogue.net in a browser on any device she gets upstream's newest client, and the first
save it writes stamps the newer `gameVersion` into her system save. From then on our pinned build
**cannot open her account at all** — not "sync fails", but a blocking English modal on a game that
otherwise looks perfectly fine. She would reasonably conclude the app is broken. Note this happens
...
4. **Until the release feed exists** (`m0stey/pokerogue-offline`, `game-<tag>` releases — NOTES-main
   open question 5, still unpublished) the honest fallback is a dialog telling her that her progress
   is safe, that she should play in the browser for now, and that the app will update itself later.
...
- The mirror's offline copy of the run is silently replaced by the server's degraded copy, so what
  she takes on the plane is worse than what her game wrote. No visible loss today (the dropped
...
the exact moment she is most attached to the result. Needs one QA pass: end a run while offline and
...
2. **The game came up in German by itself** (system locale): `Fortfahren / Neues Spiel / Spiel laden`.
   ...
   translation pending). Today she would see a German game inside an English wrapper.
...
5. **Daily Run offline**: the title menu still offers online-only modes, `/daily/*` answers 503, and
   nobody has seen what the game shows her. Untested.
6. **Login takes ~930 ms** with no progress feedback. Fine, but it is the slowest thing she does.
...
**Must fix before she gets it**

1. **The §3 game-version block.** Today one browser session on pokerogue.net permanently bricks the
   app for her. Needs `version.json` from `game-build`, the proxy-side detection, our own German
...
already has her Windows account, and both deserve one honest sentence on the settings page rather
than a code change.
