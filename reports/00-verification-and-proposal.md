...
| `currentVersion.txt`; `--clear-cache` deletes under `%APPDATA%\PokeRogue` | CONFIRMED. Game files actually live in `<install dir>\resources\game`. clear-cache spares Local Storage but deletes Cookies (logs the user out) | `src/main.js:141-172`, `src/file_tab.js:66-96` |
...
- Pro: one save store, one game build, no seeding step, tid/sid always correct, version gating disappears unless the user plays in a plain browser; the user never sees a login screen twice.
- Con: we build and host `game.zip` ourselves (GitHub Actions like the fork's, ~10 min per upstream release); the proxy must stub account endpoints offline; if the user plays in the browser in parallel, the online-vs-mirror conflict still exists (handled by the reconcile).
...
**Recommendation: Option B.** It is the only option where "the user plays as before" and "never lose progress" do not fight each other, and the failure cases are all observable in one place.
...
3. The user plays in a plain browser in parallel. Mitigation: three-way reconcile + dialogue; the app should become the only way to play.
