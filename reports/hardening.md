...
project got a bit too big — I want a usable version for the user that's stable and secure"* — and
...
answer offline tore down the game-over screen at the exact moment the user is most attached to the
result.
...
Slot and `clientSessionId` are deliberately **not** validated: an error here costs the user the end of a
...
One browser session on pokerogue.net is enough to cause it. We cannot prevent it — the save is the user's,
...
- **`src/main`**: one German dialog, informal, no technical terms — *"PokéRogue hat eine neue Version
  ...
  Shown **at most once per app start**, from any of the three things that mean the same thing to the user:
...
either tag is not a plain `x.y.z[.w]` — a wrong "there is a new version" sends the user looking for an
...
(the offline login does not verify the password; anyone with access to the user Windows account has
...
the label the game itself shows the user under Menü → Statistiken, so it is the label we use.
