...
9. **`session/newclear` answers `200` with the bare JSON `false`**; `account/register`, `daily/*`
    ...
    the game-over screen at the exact moment she is most attached to the result. The real server
    ...
    arguments are deliberately **not** validated: an error here costs her the end of a run, and
