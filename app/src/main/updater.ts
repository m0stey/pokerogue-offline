...
//   * if Windows says this is a mobile connection, we ask first (once, if she says "always")
...
export class Updater {
  ...
  async checkNow(reason: string): Promise<void> {
    ...
    try {
      ...
      // An update we cannot fetch is never worth a message: she just keeps playing.
  ...
  private async allowedToDownload(release: ReleaseInfo): Promise<boolean> {
    ...
    if (policy === "never") {
      this.deps.log.info("mobile connection: skipping the download as she asked");
      return false;
    }
  ...
  private async downloadAndStage(release: ReleaseInfo): Promise<void> {
    ...
    this.deps.log.info("new game files are ready and will be used the next time she starts the game", {
      tag: release.tag,
    });
  }
