...
interface UiSession {
  ...
  onClosed?: () => void;
  /** While true the window refuses to close - used by the question she must answer. */
  mustAnswer?: boolean;
}
...
function openUi(opts: OpenUiOptions): BrowserWindow {
  ...
  win.on("close", (event) => {
    ...
    if (s?.mustAnswer) {
      event.preventDefault(); // she has to pick one; there is no safe silent default
      win.focus();
    }
  });
  win.on("closed", () => {
  ...
  return new Promise<ConflictAnswer>((resolve, reject) => {
    ...
    const session: UiSession = {
      ...
      onSubmit: (value) => {
        ...
        c.log.info("she chose which progress to keep", { keep, remember });
...
// ---------------------------------------------------------------------------
// "Saving your progress online..." splash while she waits at quit
// ---------------------------------------------------------------------------
