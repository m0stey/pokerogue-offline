...
// src/main/wiring.ts is the only file that imports src/proxy and src/sync. While those modules do
// not exist yet it simply is not built, and index.ts falls back to its stand-in server.
if (existsSync("src/main/wiring.ts")) {
  try {
    ...
    console.warn("wiring skipped: src/proxy and src/sync are not there yet - the app will use its stand-in server");
  }
}
